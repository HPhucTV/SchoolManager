...
export default function StudentSettingsPage() {
    ...
    useEffect(() => {
        ...
        if (userData) {
            try {
                ...
                // Initialize notification settings from API response
                if (user.email_enabled !== undefined) {
                    setNotificationSettings({
                        email_enabled: user.email_enabled,
                        notify_assignments: user.notify_assignments,
                        notify_activities: user.notify_activities,
                        notify_surveys: user.notify_surveys
                    });
                }
            } catch (e) {
    ...
    const handleNotificationToggle = async (key: keyof typeof notificationSettings) => {
        ...
        if (!token) {
            console.error('No token found for notification update.');
            return;
        }
        ...
        try {
            ...
            if (!response.ok) {
                ...
                throw new Error(errorData.detail || 'Không thể cập nhật cài đặt thông báo');
            }
            ...
            alert('Lỗi khi lưu cài đặt thông báo: ' + (err instanceof Error ? err.message : 'Unknown error'));
        }
    };
                        ...
                        {navItems.map((item) => (
                            <button
                                ...
                                style={{
                                    ...
                                    background: activeTab === item.id ? 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)' : 'transparent',
                                    color: activeTab === item.id ? 'white' : '#4b5563',
                                    boxShadow: activeTab === item.id ? '0 4px 14px rgba(59, 130, 246, 0.4)' : 'none',
                                }}
                            >
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '24px', marginBottom: '32px' }}>
                                <div style={{ position: 'relative' }}>
                                    ...
                                    <div style={{
                                        ...
                                        border: avatarPreview ? '3px solid #3b82f6' : 'none',
                                    }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'flex', gap: '8px', marginTop: '8px' }}>
                                        <button
                                            onClick={handleAvatarClick}
                                            style={{ fontSize: '14px', color: '#3b82f6', fontWeight: 500, background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
                                        >
                                            Đổi ảnh đại diện
                                        </button>
                                        {avatarPreview && (
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
                                ...
                                <div style={{
                                    ...
                                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                                        <input
                                            type="checkbox"
                                            checked={notificationSettings.notify_assignments}
                                            onChange={() => handleNotificationToggle('notify_assignments')}
                                            style={{ opacity: 0, width: 0, height: 0 }}
                                        />
                                        <span style={{
                                            position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0,
                                            backgroundColor: notificationSettings.notify_assignments ? '#8b5cf6' : '#e5e7eb',
                                ...
                                <div style={{
                                    ...
                                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                                        <input
                                            type="checkbox"
                                            checked={notificationSettings.notify_activities}
                                            onChange={() => handleNotificationToggle('notify_activities')}
                                            style={{ opacity: 0, width: 0, height: 0 }}
                                        />
                                        <span style={{
                                            position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0,
                                            backgroundColor: notificationSettings.notify_activities ? '#8b5cf6' : '#e5e7eb',
                                            borderRadius: '34px', transition: '.4s'
                                        }} />
                                        <span style={{
                                            position: 'absolute', content: '""', height: '18px', width: '18px',
                                ...
                                <div style={{
                                    ...
                                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                                        <input
                                            type="checkbox"
                                            checked={notificationSettings.notify_surveys}
                                            onChange={() => handleNotificationToggle('notify_surveys')}
                                            style={{ opacity: 0, width: 0, height: 0 }}
                                        />
                                        <span style={{
                                            position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0,
                                            backgroundColor: notificationSettings.notify_surveys ? '#8b5cf6' : '#e5e7eb',
                                            borderRadius: '34px', transition: '.4s'
                                        }} />
                                        <span style={{
                                            position: 'absolute', content: '""', height: '18px', width: '18px',
                                ...
                                <div style={{
                                    ...
                                    <label style={{ position: 'relative', display: 'inline-block', width: '44px', height: '24px' }}>
                                        ...
                                        <span style={{
                                            position: 'absolute', cursor: 'pointer', top: 0, left: 0, right: 0, bottom: 0,
                                            backgroundColor: notificationSettings.email_enabled ? '#10b981' : '#e5e7eb',
                                            borderRadius: '34px', transition: '.4s'
                                        }} />
                                        <span style={{
                                            position: 'absolute', content: '""', height: '18px', width: '18px',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '16px' }}>
                        ...
                        <button
                            ...
                            style={{
                                ...
                                background: saving ? '#d1d5db' : 'linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)',
